...
jest.mock("../../utils/api", () => ({
  api: {
    useContext: () => ({
      paMail: {
        getAllMailByNick: { invalidate: mockInvalidate, refetch: mockRefetch },
      },
    }),
    paMail: {
...
describe("Mail page", () => {
  ...
  it("renders mail table when data is loaded", () => {
    setupWithMail({
      mail: [
        { id: 1, seen: 1, news: "Hello", header: "Test", sentTo: 1, time: 0 },
      ],
    });
    render(<Mail />);
  ...
  it('renders "Mark all as seen" button when there are unseen emails', () => {
    setupWithMail({
      mail: [
        { id: 1, seen: 0, news: "Hello", header: "Test", sentTo: 1, time: 0 },
      ],
    });
    render(<Mail />);
  ...
  it("calls invalidate and refetch on markAsSeen success", async () => {
    ...
    await act(async () => {
      await capturedMarkSeenOnSuccess?.();
    });

...
jest.mock("../../components/ui", () => ({
  AdvancedDataTable: ({ caption, columns, data }: MockTableProps) => {
    ...
    if (actionsCol && typeof actionsCol.accessor === "function" && data[0]) {
      actionsOutput = (
        actionsCol.accessor as (row: Record<string, unknown>) => React.ReactNode
      )(data[0]);
    }
    return (
...
jest.mock("../../components/ui/tables/RankingActions", () => ({
  __esModule: true,
  default: ({ playerNick }: { playerNick: string }) => (
    <div data-testid="ranking-actions">{playerNick}</div>
  ),
}));
...
describe("Ranking page", () => {
  ...
  it("renders AdvancedDataTable when player and ranking data are loaded", () => {
    setupAuthenticated([
      createMockPaPlayer(),
      createMockPaPlayer({ id: 2, nick: "Player2" }),
    ]);
    render(<RankingPage />);

...
describe('Production page', () => {
  ...
  it('renders barracks message when c_airport is 0', () => {
    mockUsePlayerData.mockReturnValue({ paPlayer: createPlayer({ c_airport: 0 }), isAuthenticated: true });
    render(<ProductionPage />);
    expect(screen.getByTestId('render-message')).toBeInTheDocument();
    expect(screen.getByTestId('render-message')).toHaveTextContent('Production');
  });

  it('renders barracks message when c_airport > 1', () => {
    mockUsePlayerData.mockReturnValue({ paPlayer: createPlayer({ c_airport: 2 }), isAuthenticated: true });
    render(<ProductionPage />);
    expect(screen.getByTestId('render-message')).toBeInTheDocument();
  });

  it('renders Production component when c_airport is 1', () => {
    mockUsePlayerData.mockReturnValue({ paPlayer: createPlayer({ c_airport: 1 }), isAuthenticated: true });
    render(<ProductionPage />);
  ...
  it('does not render Production component when c_airport is 0', () => {
    mockUsePlayerData.mockReturnValue({ paPlayer: createPlayer({ c_airport: 0 }), isAuthenticated: true });
    render(<ProductionPage />);
    expect(screen.queryByTestId('production-component')).not.toBeInTheDocument();
  });
});

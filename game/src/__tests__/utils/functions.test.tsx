...
const createMockPlayer = (overrides: Partial<PaPlayerBase> = {}): PaPlayerBase => ({
  id: 1,
  nick: 'Test',
  crystal: 5000,
  metal: 3000,
...
describe('canAffordToTrain', () => {
  ...
  it('considers land when considerLand is true', () => {
    const player = createMockPlayer({ crystal: 5000, metal: 3000, ui_roids: 2 });
    expect(canAffordToTrain([player], 100, 50, 5, true)).toBe(false);
  });

  it('allows training within land limit when considerLand is true', () => {
    const player = createMockPlayer({ crystal: 5000, metal: 3000, ui_roids: 10 });
...
describe('renderIncomeData', () => {
  it('returns chart data with correct labels', () => {
    ...
    const result = renderIncomeData(player as Parameters<typeof renderIncomeData>[0]);
